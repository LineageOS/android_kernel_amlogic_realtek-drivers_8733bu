...
#include "../hal_halmac.h"	/* GET_RX_DESC_XXX_8733B() */
#include "rtl8733b.h"
...
static const struct hw_port_reg port_cfg[] = {
	/*port 0*/
	{
	.net_type = (REG_CR_8733B + 2),
	.net_type_shift = 0,
	.macaddr = REG_MACID_8733B,
	.bssid = REG_BSSID_8733B,
	.bcn_ctl = REG_BCN_CTRL_8733B,
	.tsf_rst = REG_DUAL_TSF_RST,
	.tsf_rst_bit = BIT_TSFTR_RST_8733B,
	.bcn_space = REG_MBSSID_BCN_SPACE_8733B,
	.bcn_space_shift = 0,
	.bcn_space_mask = 0xffff,
	.ps_aid = REG_BCN_PSR_RPT_8733B,
	.ta = REG_TRANSMIT_ADDRSS_0_8733B,
	},
	/*port 1*/
	{
	.net_type = (REG_CR_8733B + 2),
	.net_type_shift = 2,
	.macaddr = REG_MACID1_8733B,
	.bssid = REG_BSSID1_8733B,
	.bcn_ctl = REG_BCN_CTRL_CLINT0_8733B,
	.tsf_rst = REG_DUAL_TSF_RST,
	.tsf_rst_bit = BIT_TSFTR_CLI0_RST_8733B,
	.bcn_space = REG_MBSSID_BCN_SPACE_8733B,
	.bcn_space_shift = 16,
	.bcn_space_mask = 0xfff,
	.ps_aid = REG_BCN_PSR_RPT1_8733B,
	.ta = REG_TRANSMIT_ADDRSS_1_8733B,
	},
	/*port 2*/
	{
	.net_type =  REG_CR_EXT_8733B,
	.net_type_shift = 0,
	.macaddr = REG_MACID2_8733B,
	.bssid = REG_BSSID2_8733B,
	.bcn_ctl = REG_BCN_CTRL_CLINT1_8733B,
	.tsf_rst = REG_DUAL_TSF_RST,
	.tsf_rst_bit = BIT_TSFTR_CLI1_RST_8733B,
	.bcn_space = REG_MBSSID_BCN_SPACE2_8733B,
	.bcn_space_shift = 0,
	.bcn_space_mask = 0xfff,
	.ps_aid = REG_BCN_PSR_RPT2_8733B,
	.ta = REG_TRANSMIT_ADDRSS_2_8733B,
	},
	/*port 3*/
	{
	.net_type =  REG_CR_EXT_8733B,
	.net_type_shift = 2,
	.macaddr = REG_MACID3_8733B,
	.bssid = REG_BSSID3_8733B,
	.bcn_ctl = REG_BCN_CTRL_CLINT2_8733B,
	.tsf_rst = REG_DUAL_TSF_RST,
	.tsf_rst_bit = BIT_TSFTR_CLI2_RST_8733B,
	.bcn_space = REG_MBSSID_BCN_SPACE2_8733B,
	.bcn_space_shift = 16,
	.bcn_space_mask = 0xfff,
	.ps_aid = REG_BCN_PSR_RPT3_8733B,
	.ta = REG_TRANSMIT_ADDRSS_3_8733B,
	},
	/*port 4*/
	{
	.net_type =  REG_CR_EXT_8733B,
	.net_type_shift = 4,
	.macaddr = REG_MACID4_8733B,
	.bssid = REG_BSSID4_8733B,
	.bcn_ctl = REG_BCN_CTRL_CLINT3_8733B,
	.tsf_rst = REG_DUAL_TSF_RST,
	.tsf_rst_bit = BIT_TSFTR_CLI3_RST_8733B,
	.bcn_space = REG_MBSSID_BCN_SPACE3_8733B,
	.bcn_space_shift = 0,
	.bcn_space_mask = 0xfff,
	.ps_aid = REG_BCN_PSR_RPT4_8733B,
	.ta = REG_TRANSMIT_ADDRSS_4_8733B,
	},
};
...
static void read_chip_version(PADAPTER adapter)
{
	...
	value32 = rtw_read32(adapter, REG_SYS_CFG1_8733B);
	hal->version_id.ICType = CHIP_8733B;
	hal->version_id.ChipType = ((value32 & BIT_RTL_ID_8733B) ? TEST_CHIP : NORMAL_CHIP);
	hal->version_id.CUTVersion = BIT_GET_CHIP_VER_8733B(value32);
	hal->version_id.VendorType = BIT_GET_VENDOR_ID_8733B(value32);
	...
	hal->version_id.RFType = ((value32 & BIT_RF_TYPE_ID_8733B) ? RF_TYPE_2T2R : RF_TYPE_1T1R);

	/* LDO mode : 1, SPS mode : 0 */
	hal->RegulatorMode = ((value32 & BIT_SPSLDO_SEL_8733B) ? RT_LDO_REGULATOR : RT_SWITCHING_REGULATOR);

	value32 = rtw_read32(adapter, REG_SYS_STATUS1_8733B);
	hal->version_id.ROMVer = BIT_GET_RF_RL_ID_8733B(value32);

	/* For multi-function consideration. */
	hal->MultiFunc = RT_MULTI_FUNC_NONE;
	value32 = rtw_read32(adapter, REG_WL_BT_PWR_CTRL_8733B);
	hal->MultiFunc |= ((value32 & BIT_WL_FUNC_EN_8733B) ? RT_MULTI_FUNC_WIFI : 0);
	hal->MultiFunc |= ((value32 & BIT_BT_FUNC_EN_8733B) ? RT_MULTI_FUNC_BT : 0);
	hal->PolarityCtl = ((value32 & BIT_WL_HWPDN_SL_8733B) ? RT_POLARITY_HIGH_ACT : RT_POLARITY_LOW_ACT);
...
static void Hal_EfuseParseEEPROMVer(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (_TRUE == mapvalid)
		hal->EEPROMVersion = map[EEPROM_VERSION_8733B];
	else
		hal->EEPROMVersion = 1;
...
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 tpt_mode = (map[EEPROM_TX_PWR_CALIBRATE_RATE_8733B] & 0xF0) >> 4;
	int ret = _FAIL;
...
parse_tpt_mode_done:
	...
	if ((_TRUE == mapvalid) && (map[EEPROM_RF_BOARD_OPTION_8733B] != 0xFF))
		hal->EEPROMRegulatory = map[EEPROM_RF_BOARD_OPTION_8733B] & 0x3; /* bit0~1 */
	else
...
static void Hal_EfuseParseBoardType(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if ((_TRUE == mapvalid) && (map[EEPROM_RF_BOARD_OPTION_8733B] != 0xFF))
		hal->InterfaceSel = (map[EEPROM_RF_BOARD_OPTION_8733B] & 0xE0) >> 5;
	else
...
static void Hal_EfuseParseBTCoexistInfo(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if ((_TRUE == mapvalid) && (map[EEPROM_RF_BOARD_OPTION_8733B] != 0xFF)) {
		/* 0xc1[7:5] = 0x01, combo module */
		if (((map[EEPROM_RF_BOARD_OPTION_8733B] & 0xe0) >> 5) == 0x01)
	...
	hal->EEPROMBluetoothType = BT_RTL8733B;

	setting = map[EEPROM_RF_BT_SETTING_8733B];
	if ((_TRUE == mapvalid) && (setting != 0xFF)) {
		hal->EEPROMBluetoothAntNum = setting & BIT(0);
		/* 0xC3[6] is only used by 8723B, resverd at 8733B */
...
static void Hal_EfuseParseXtal(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if ((_TRUE == mapvalid) && map[EEPROM_XTAL_B9_8733B] != 0xFF)
		hal->crystal_cap = map[EEPROM_XTAL_B9_8733B];
	else
		hal->crystal_cap = EEPROM_Default_CrystalCap_8733B;
...
static void Hal_EfuseParseThermalMeter(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if ((_TRUE == mapvalid) && (map[EEPROM_THERMAL_METER_8733B] != 0xFF)) {
		hal->eeprom_thermal_meter = map[EEPROM_THERMAL_METER_8733B];
	} else {
...
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 trx_path_bmp = map[EEPROM_RF_ANTENNA_OPT_8733B];
	...
	case 0x22: /* 1T1R - TX:B,	RX:B */
		...
		RTW_INFO("EEPROM efuse[0x%x]=0x%02x is valid\n"
			, EEPROM_RF_ANTENNA_OPT_8733B, trx_path_bmp);
		break;
	default:
...
static void Hal_EfuseParseCustomerID(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (_TRUE == mapvalid)
		hal->EEPROMCustomerID = map[EEPROM_CustomID_8733B];
	else
...
static void hal_ReadPAType(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (mapvalid) {
		...
		if (GetRegAmplifierType2G(adapter) == 0) {
			hal->PAType_2G = ReadLE1Byte(&map[EEPROM_2G_5G_PA_TYPE_8733B]);
			hal->LNAType_2G = ReadLE1Byte(&map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8733B]);
		...
		if (GetRegAmplifierType5G(adapter) == 0) {
			hal->PAType_5G = ReadLE1Byte(&map[EEPROM_2G_5G_PA_TYPE_8733B]);
			hal->LNAType_5G = ReadLE1Byte(&map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8733B]);
...
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 extTypePA_2G_A = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8733B] & BIT2) >> 2;
	u8 extTypePA_2G_B = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8733B] & BIT6) >> 6;
	u8 extTypePA_5G_A = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8733B] & BIT2) >> 2;
	u8 extTypePA_5G_B = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8733B] & BIT6) >> 6;
	u8 extTypeLNA_2G_A = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8733B] & (BIT1 | BIT0)) >> 0;
	u8 extTypeLNA_2G_B = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8733B] & (BIT5 | BIT4)) >> 4;
	u8 extTypeLNA_5G_A = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8733B] & (BIT1 | BIT0)) >> 0;
	u8 extTypeLNA_5G_B = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8733B] & (BIT5 | BIT4)) >> 4;
...
static u8 Hal_ReadRFEType(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (mapvalid) {
		/* check efuse map */
		hal->rfe_type = ReadLE1Byte(&map[EEPROM_RFE_OPTION_8733B]);
	...
	RTW_ERR("cmd: insmod rtl8733bx.ko rtw_RFE_type=<rfe_type>\n\n");
...
static void Hal_ReadUsbModeSwitch(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (_TRUE == mapvalid)
		hal->EEPROMUsbSwitch = (map[EEPROM_USB_MODE_8733BU] & BIT0);
	else
...
static void hal_read_usb_pid_vid(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (_TRUE == mapvalid) {
		/* VID, PID */
		hal->EEPROMVID = ReadLE2Byte(&map[EEPROM_VID_8733BU]);
		hal->EEPROMPID = ReadLE2Byte(&map[EEPROM_PID_8733BU]);
	} else {
 ...
 */
u8 rtl8733b_read_efuse(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	u8 val8;
	u8 *efuse_map = NULL;
	...
	/* 1. Read registers to check hardware eFuse available or not */
	val8 = rtw_read8(adapter, REG_SYS_EEPROM_CTRL_8733B);
	hal->EepromOrEfuse = (val8 & BIT_EERPOMSEL_8733B) ? _TRUE : _FALSE;
	hal->bautoload_fail_flag = (val8 & BIT_AUTOLOAD_SUS_8733B) ? _FALSE : _TRUE;
	if (hal->bautoload_fail_flag == _TRUE)
		RTW_ERR("%s: HW %s AUTO LOAD FAIL!!\n", __func__, hal->EepromOrEfuse ? "EEPROM" : "EFUSE");
	/*
	 * In 8733B, bautoload_fail_flag is used to present eFuse map is valid
...
void rtl8733b_run_thread(PADAPTER adapter)
{
}

void rtl8733b_cancel_thread(PADAPTER adapter)
{
}
...
#endif
	rtw_write16(adapter, REG_BCN_CTRL_8733B, val16);

	/* TBTT setup time */
	rtw_write8(adapter, REG_TBTT_PROHIBIT_8733B, TBTT_PROHIBIT_SETUP_TIME);

	/* TBTT hold time: 0x540[19:8] */
	rtw_write8(adapter, REG_TBTT_PROHIBIT_8733B + 1, TBTT_PROHIBIT_HOLD_TIME_STOP_BCN & 0xFF);
	rtw_write8(adapter, REG_TBTT_PROHIBIT_8733B + 2,
		(rtw_read8(adapter, REG_TBTT_PROHIBIT_8733B + 2) & 0xF0) | (TBTT_PROHIBIT_HOLD_TIME_STOP_BCN >> 8));

	rtw_write8(adapter, REG_DRVERLYINT_8733B, DRIVER_EARLY_INT_TIME_8733B); /* 5ms */
	rtw_write8(adapter, REG_BCNDMATIM_8733B, BCN_DMA_ATIME_INT_TIME_8733B); /* 2ms */

	/*
	 * Suggested by designer timchen. Change beacon AIFS to the largest number
	 * beacause test chip does not contension before sending beacon.
	 */
	rtw_write16(adapter, REG_BCNTCFG_8733B, 0x4413);
}
...
static void beacon_function_enable(PADAPTER adapter, u8 Enable, u8 Linked)
{
	...
	/* port0 */
	bcn_ctrl_reg = REG_BCN_CTRL_8733B;
	val8  = BIT_DIS_TSF_UDT_8733B | BIT_EN_BCN_FUNCTION_8733B;
#ifdef CONFIG_CONCURRENT_MODE
	/* port1 */
	if (adapter->hw_port == HW_PORT1) {
		bcn_ctrl_reg = REG_BCN_CTRL_CLINT0_8733B;
		val8 = BIT_CLI0_DIS_TSF_UDT_8733B | BIT_CLI0_EN_BCN_FUNCTION_8733B;
	}
#endif

	rtw_write8(adapter, bcn_ctrl_reg, val8);
	rtw_write8(adapter, REG_RD_CTRL_8733B + 1, 0x6F);
}
...
static void set_beacon_related_registers(PADAPTER adapter)
{
	 ...
	 * ATIM window
	 */
	rtw_write8(adapter, REG_ATIMWND_GROUP1_8733B, 2);
	...
	rtw_write8(adapter, REG_SLOT_8733B, 0x09);

	/* Reset TSF Timer to zero */
	val8 = BIT_TSFTR_RST_8733B;
#ifdef CONFIG_CONCURRENT_MODE
	if (adapter->hw_port == HW_PORT1)
		val8 = BIT_TSFTR_CLI0_RST_8733B;
#endif
	rtw_write8(adapter, REG_DUAL_TSF_RST_8733B, val8);
	val8 = BIT_TSFTR_RST_8733B;
	rtw_write8(adapter, REG_DUAL_TSF_RST_8733B, val8);

	rtw_write8(adapter, REG_RXTSF_OFFSET_CCK_8733B, 0x50);
	rtw_write8(adapter, REG_RXTSF_OFFSET_OFDM_8733B, 0x50);
...
static void xmit_status_check(PADAPTER p)
{
	...
	txdma_status = rtw_read32(p, REG_TXDMA_STATUS_8733B);
...
static void linked_status_check(PADAPTER p)
{
	...
	rx_dma_status = rtw_read32(p, REG_RXDMA_STATUS_8733B);
...
static void set_opmode_monitor(PADAPTER adapter)
{
	...
	/* Receive all type */
	tmp_32bit = BIT_AAP_8733B | BIT_APP_PHYSTS_8733B;

	if (ndev->type == ARPHRD_IEEE80211_RADIOTAP) {
		/* Append FCS */
		tmp_32bit |= BIT_APP_FCS_8733B;
	}
	...
	tmp_8bit = rtw_read8(adapter, REG_RX_DRVINFO_SZ_8733B);
	rtw_write8(adapter, REG_RX_DRVINFO_SZ_8733B, (tmp_8bit | 0x80));

	/* Receive all data frames */
	mon->known_rxfilter = 1;
	mon->rxfilter0 = rtw_read16(adapter, REG_RXFLTMAP0_8733B);
	mon->rxfilter1 = rtw_read16(adapter, REG_RXFLTMAP1_8733B);
	mon->rxfilter2 = rtw_read16(adapter, REG_RXFLTMAP2_8733B);
	rtw_write16(adapter, REG_RXFLTMAP0_8733B, 0xFFFF);
	rtw_write16(adapter, REG_RXFLTMAP1_8733B, 0xFFFF);
...
static void set_opmode_port0(PADAPTER adapter, u8 mode)
{
	...
	case _HW_STATE_STATION_:
		...
		val8 = BIT_DIS_TSF_UDT_8733B | BIT_EN_BCN_FUNCTION_8733B;
		rtw_write8(adapter, REG_BCN_CTRL_8733B, val8);

		/* disable atim wnd(only for Port0) */
		val8 = rtw_read8(adapter, REG_HIQ_NO_LMT_EN_V2);
		val8 |= BIT_DIS_ATIM_ROOT_8733B;
		rtw_write8(adapter, REG_HIQ_NO_LMT_EN_V2, val8);
		break;
	...
	case _HW_STATE_AP_:
		...
		UpdateInterruptMask8733BE(adapter, RT_BCN_INT_MASKS, 0, 0, 0);
		 ...
		 * Reg REG_FWHW_TXQ_CTRL_8733B [2] = 1
		...
		val8 = rtw_read8(adapter, REG_FWHW_TXQ_CTRL_8733B);
		val8 |= BIT_EN_BCN_TRXRPT_8733B;
		rtw_write8(adapter, REG_FWHW_TXQ_CTRL_8733B, val8);

		/* enable BCN0 Function */
		val8 = rtw_read8(adapter, REG_BCN_CTRL_8733B);
		val8 |= BIT_EN_BCN_FUNCTION_8733B | BIT_DIS_TSF_UDT_8733B | BIT_P0_EN_TXBCN_RPT_8733B;
		val8 &= (~BIT_P0_EN_RXBCN_RPT_8733B);
		rtw_write8(adapter, REG_BCN_CTRL_8733B, val8);
		...
		/* Enable HW seq for BCN
			...
			According TX desc
		*/
		rtw_write8(adapter, REG_DUMMY_PAGE4_8733B, 0x01);

		/* enable to rx data frame */
		rtw_write16(adapter, REG_RXFLTMAP2_8733B, 0xFFFF);
		/* enable to rx ps-poll */
		val16 = rtw_read16(adapter, REG_RXFLTMAP1_8733B);
		val16 |= BIT_CTRLFLT10EN_8733B;
		rtw_write16(adapter, REG_RXFLTMAP1_8733B, val16);

		/* Beacon Control related register for first time */
		rtw_write8(adapter, REG_BCNDMATIM_8733B, 0x02); /* 2ms */

		rtw_write8(adapter, REG_ATIMWND_GROUP1_8733B, 0x0c); /* 12ms */

		rtw_write16(adapter, REG_TSFTR_SYN_OFFSET_8733B, 0x7fff); /* +32767 (~32ms) */

		/* reset TSF */
		rtw_write8(adapter, REG_DUAL_TSF_RST_8733B, BIT_TSFTR_RST_8733B);

		/* SW_BCN_SEL - Port0 */
		rtw_hal_set_hwreg(adapter, HW_VAR_DL_BCN_SEL, NULL);

		/* select BCN on port 0 */
		val8 = rtw_read8(adapter, REG_CCK_CHECK_8733B);
		val8 &= ~BIT_BCN_PORT_SEL_8733B;
		rtw_write8(adapter, REG_CCK_CHECK_8733B, val8);
		break;
	}
}
static void set_opmode_port1(PADAPTER adapter, u8 mode)
{
	...
	case _HW_STATE_STATION_:
		...
		/* disable beacon function */
		val8 = BIT_CLI0_DIS_TSF_UDT_8733B | BIT_CLI0_EN_BCN_FUNCTION_8733B;
		rtw_write8(adapter, REG_BCN_CTRL_CLINT0_8733B, val8);
		break;
...
static void hw_var_set_opmode(PADAPTER adapter, u8 mode)
{
	...
	if (isMonitor == _TRUE) {
		...
		if (backup->known_rcr) {
			...
			if (!!(backup->rcr &= BIT_APP_PHYSTS_8733B))
				rtw_halmac_config_rx_info(adapter_to_dvobj(adapter),
		...
		if (backup->known_rxfilter) {
			...
			rtw_write16(adapter, REG_RXFLTMAP0_8733B, backup->rxfilter0);
			rtw_write16(adapter, REG_RXFLTMAP1_8733B, backup->rxfilter1);
			rtw_write16(adapter, REG_RXFLTMAP2_8733B, backup->rxfilter2);
		}
#endif /* CONFIG_WIFI_MONITOR */
		isMonitor = _FALSE;
	}
	...
	/* clear crc bit */
	if (rtw_hal_rcr_check(adapter, BIT_ACRC32_8733B))
		rtw_hal_rcr_clear(adapter, BIT_ACRC32_8733B);
...
{
	if (enable)
		hw_bcn_ctrl_add(adapter, get_hw_port(adapter),
				(BIT_P0_EN_RXBCN_RPT_8733B
				 | BIT_DIS_TSF_UDT_8733B
				 | BIT_EN_BCN_FUNCTION_8733B));
	else
...
static void hw_var_set_bcn_func(PADAPTER adapter, u8 enable)
{
	...
	if (enable) {
		...
		switch (adapter->hw_port) {
		case HW_PORT0:
			val8 =  BIT_EN_BCN_FUNCTION_8733B | BIT_P0_EN_TXBCN_RPT_8733B;
			hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_P0_EN_RXBCN_RPT_8733B);
			break;
#ifdef CONFIG_CONCURRENT_MODE
		case HW_PORT1:
			val8 =  BIT_CLI0_EN_BCN_FUNCTION_8733B;
			hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_CLI0_EN_RXBCN_RPT_8733B);
			break;
		case HW_PORT2:
			val8 =  BIT_CLI1_EN_BCN_FUNCTION_8733B;
			hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_CLI1_EN_RXBCN_RPT_8733B);
			break;
		case HW_PORT3:
			val8 =  BIT_CLI2_EN_BCN_FUNCTION_8733B;
			hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_CLI2_EN_RXBCN_RPT_8733B);
			break;
		case HW_PORT4:
			val8 =  BIT_CLI3_EN_BCN_FUNCTION_8733B;
			hw_bcn_ctrl_clr(adapter, get_hw_port(adapter), BIT_CLI3_EN_RXBCN_RPT_8733B);
			break;
#endif /* CONFIG_CONCURRENT_MODE */
		default:
		...
		case HW_PORT0:
			...
			if (GET_HAL_DATA(adapter)->EEPROMBluetoothCoexist)
				val8 = BIT_P0_EN_TXBCN_RPT_8733B;
...
static void hw_var_set_mlme_disconnect(PADAPTER adapter)
{
	...
	if (rtw_mi_check_status(adapter, MI_LINKED) == _FALSE)
		...
		rtw_write16(adapter, REG_RXFLTMAP2_8733B, 0);

#ifdef CONFIG_CONCURRENT_MODE
	if (adapter->hw_port == HW_PORT1) {
		/* reset TSF1(CLINT0) */
		rtw_write8(adapter, REG_DUAL_TSF_RST_8733B, BIT_TSFTR_CLI0_RST_8733B);

		/* disable update TSF1(CLINT0) */
		rtw_iface_disable_tsf_update(adapter);

		/* disable Port1's beacon function */
		val8 = rtw_read8(adapter, REG_BCN_CTRL_CLINT0_8733B);
		val8 &= ~BIT_CLI0_EN_BCN_FUNCTION_8733B;
		rtw_write8(adapter, REG_BCN_CTRL_CLINT0_8733B, val8);
	} else
#endif
	...
	if (MSTATE_STA_LD_NUM(&mstate) == 0 && MSTATE_STA_LG_NUM(&mstate) == 0)
		rtl8733b_rx_tsf_addr_filter_config(adapter, 0);
...
static void hw_var_set_mlme_sitesurvey(PADAPTER adapter, u8 enable)
{
	...
	if (enable) {
		...
		rtw_write16(adapter, REG_RXFLTMAP2_8733B, value_rxfltmap2);
...
static void hw_var_set_mlme_join(PADAPTER adapter, u8 type)
{
	...
	if (type == 0) {
		...
		/*
		 * for 8733B, must enable BCN function if BIT_CBSSID_BCN_8733B(bit 7) of REG_RCR(0x608) is enable to recv BSSID bcn
		 */
		hw_var_set_bcn_func(adapter, _TRUE);

		/* update TSF only BSSID match for station mode */
		rtl8733b_rx_tsf_addr_filter_config(adapter, BIT_NAN_RX_FILTER_EN_8733B | BIT_NAN_RX_CHKBSSID_8733B);
		#ifdef CONFIG_CLIENT_PORT_CFG
		...
		if (rtw_mi_check_status(adapter, MI_LINKED) == _FALSE)
			rtw_write16(adapter, REG_RXFLTMAP2_8733B, 0x00);
	...
	val16 = BIT_LRL_8733B(RetryLimit) | BIT_SRL_8733B(RetryLimit);
	rtw_write16(adapter, REG_RETRY_LIMIT_8733B, val16);
#else /* !CONFIG_CONCURRENT_MODE */
	if (type == 0) {
		/* prepare to join */
		...
		/*
		 * for 8733B, must enable BCN function if BIT_CBSSID_BCN_8733B(bit 7) of REG_RCR(0x608) is enabled to recv BSSID bcn
		 */
		hw_var_set_bcn_func(adapter, _TRUE);
...
static void hw_var_set_acm_ctrl(PADAPTER adapter, u8 ctrl)
{
	...
	if (ctrl) {
		hwctrl |= BIT_ACMHWEN_8733B;

		if (ctrl & BIT(1)) /* BE */
			hwctrl |= BIT_BEQ_ACM_EN_8733B;
		else
			hwctrl &= (~BIT_BEQ_ACM_EN_8733B);

		if (ctrl & BIT(2)) /* VI */
			hwctrl |= BIT_VIQ_ACM_EN_8733B;
		else
			hwctrl &= (~BIT_VIQ_ACM_EN_8733B);

		if (ctrl & BIT(3)) /* VO */
			hwctrl |= BIT_VOQ_ACM_EN_8733B;
		else
			hwctrl &= (~BIT_VOQ_ACM_EN_8733B);
	}

	RTW_INFO("[HW_VAR_ACM_CTRL] Write 0x%02X\n", hwctrl);
	rtw_write8(adapter, REG_ACMHWCTRL_8733B, hwctrl);
}
...
static void hw_var_set_sec_dk_cfg(PADAPTER adapter, u8 enable)
{
	...
	u8 reg_scr = rtw_read8(adapter, REG_SECCFG_8733B);

	if (enable) {
		/* Enable default key related setting */
		reg_scr |= BIT_TXBCUSEDK_8733B;
		if (sec->dot11AuthAlgrthm != dot11AuthAlgrthm_8021X)
			reg_scr |= BIT_RXUHUSEDK_8733B | BIT_TXUHUSEDK_8733B;
	} else {
		/* Disable default key related setting */
		reg_scr &= ~(BIT_RXBCUSEDK_8733B | BIT_TXBCUSEDK_8733B | BIT_RXUHUSEDK_8733B | BIT_TXUHUSEDK_8733B);
	}

	rtw_write8(adapter, REG_SECCFG_8733B, reg_scr);

	RTW_INFO("%s: [HW_VAR_SEC_DK_CFG] 0x%x=0x%08x\n", __FUNCTION__,
		 REG_SECCFG_8733B, rtw_read32(adapter, REG_SECCFG_8733B));
}
...
static void hw_var_set_bcn_valid(PADAPTER adapter)
{
	...
	val8 = rtw_read8(adapter, REG_DWBCN0_CTRL_8733B + 2);
	val8 |= BIT(0);
...
static void hw_var_set_ack_preamble(PADAPTER adapter, u8 bShortPreamble)
{
	...
	val8 = rtw_read8(adapter, REG_WMAC_TRXPTCL_CTL_8733B + 2);
	val8 |= BIT(4) | BIT(5);
...
void hw_var_set_dl_rsvd_page(PADAPTER adapter, u8 mstatus)
{
	...
	if (mstatus == RT_MEDIA_CONNECT) {
		...
		/* We should set AID, correct TSF, HW seq enable before set JoinBssReport to Fw in 8733B. */
		rtw_write16(adapter, port_cfg[hw_port].ps_aid, (0xF800 | pmlmeinfo->aid));

		/* Enable SW TX beacon */
		v8 = rtw_read8(adapter, REG_CR_8733B + 1);
		restore[0] = v8;
		v8 |= (BIT_ENSWBCN_8733B >> 8);
		rtw_write8(adapter, REG_CR_8733B + 1, v8);
		...
		val8 = rtw_read8(adapter, REG_BCN_CTRL_8733B);
		restore[1] = val8;
		val8 &= ~BIT_EN_BCN_FUNCTION_8733B;
		val8 |= BIT_DIS_TSF_UDT_8733B;
		rtw_write8(adapter, REG_BCN_CTRL_8733B, val8);

#if 0
		/* Set FWHW_TXQ_CTRL 0x422[6]=0 to tell Hw the packet is not a real beacon frame. */
		RegFwHwTxQCtrl = rtw_read8(adapter, REG_FWHW_TXQ_CTRL_8733B + 2);
		...
		rtw_write8(adapter, REG_FWHW_TXQ_CTRL_8733B + 2, RegFwHwTxQCtrl);
#endif
		...
		rtw_write8(adapter,  REG_CR_8733B + 1, restore[0]);
#if 0
		/*
...
u8 rtl8733b_sethwreg(PADAPTER adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 ret = _SUCCESS;
	u8 val8;
...
*/
	...
	case HW_VAR_TXPAUSE:
		rtw_write8(adapter, REG_TXPAUSE_8733B, *val);
		break;
	...
	case HW_VAR_RCR:
		ret = rtl8733b_rcr_config(adapter, *((u32 *)val));
		break;

	case HW_VAR_SLOT_TIME:
		rtw_write8(adapter, REG_SLOT_8733B, *val);
		break;
...
*/
	...
	case HW_VAR_CAM_INVALID_ALL:
		val32 = BIT_SECCAM_POLLING_8733B | BIT_KEYCAM_CLEAR_8733B;
		rtw_write32(adapter, REG_KEYCAMCMD_8733B, val32);
		break;

	case HW_VAR_AC_PARAM_VO:
		rtw_write32(adapter, REG_EDCA_VO_PARAM_8733B, *(u32 *)val);
		break;

	case HW_VAR_AC_PARAM_VI:
		rtw_write32(adapter, REG_EDCA_VI_PARAM_8733B, *(u32 *)val);
		break;

	case HW_VAR_AC_PARAM_BE:
		hal->ac_param_be = *(u32 *)val;
		rtw_write32(adapter, REG_EDCA_BE_PARAM_8733B, *(u32 *)val);
		break;

	case HW_VAR_AC_PARAM_BK:
		rtw_write32(adapter, REG_EDCA_BK_PARAM_8733B, *(u32 *)val);
		break;
...
*/
	...
	case HW_VAR_AMPDU_FACTOR: {
		...
		rtw_write32(adapter, REG_AMPDU_MAX_LENGTH_8733B, AMPDULen);
	}
	break;
...
*/
	case HW_VAR_H2C_FW_PWRMODE:
		rtl8733b_set_FwPwrMode_cmd(adapter, *val);
		break;

	case HW_VAR_H2C_FW_PWRMODE_RFON_CTRL:
		rtl8733b_set_FwPwrMode_rfon_ctrl_cmd(adapter, *val);
		break;
...
*/
...
#ifdef CONFIG_WOWLAN
		rtl8733b_set_fw_pwrmode_inips_cmd_wowlan(adapter, *val);
#endif /* CONFIG_WOWLAN */
		break;
	...
	case HW_VAR_DL_RSVD_PAGE:
		...
		if (check_fwstate(&adapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
			rtl8733b_download_BTCoex_AP_mode_rsvd_page(adapter);
#endif
		break;
#ifdef CONFIG_P2P_PS
...
*/
	case HW_VAR_FIFO_CLEARN_UP: {
		...
		/* pause tx */
		rtw_write8(adapter, REG_TXPAUSE_8733B, 0xff);

		/* keep hw sn */
		if (adapter->xmitpriv.hw_ssn_seq_no == 1)
			reg_hw_ssn = REG_HW_SEQ1_8733B;
		else if (adapter->xmitpriv.hw_ssn_seq_no == 2)
			reg_hw_ssn = REG_HW_SEQ2_8733B;
		else if (adapter->xmitpriv.hw_ssn_seq_no == 3)
			reg_hw_ssn = REG_HW_SEQ3_8733B;
		else
			reg_hw_ssn = REG_HW_SEQ0_8733B;

		adapter->xmitpriv.nqos_ssn = rtw_read16(adapter, reg_hw_ssn);

		if (pwrpriv->bkeepfwalive != _TRUE) {
			/* RX DMA stop */
			val32 = rtw_read32(adapter, REG_RXPKT_NUM_8733B);
			val32 |= BIT_RW_RELEASE_EN;
			rtw_write32(adapter, REG_RXPKT_NUM_8733B, val32);
			do {
				val32 = rtw_read32(adapter, REG_RXPKT_NUM_8733B);
				val32 &= BIT_RXDMA_IDLE_8733B;
				if (val32)
					break;
	...
	case HW_VAR_CHECK_TXBUF: {
		...
		rtylmtorg = rtw_read16(adapter, REG_RETRY_LIMIT_8733B);
		...
		rtw_write16(adapter, REG_RETRY_LIMIT_8733B, rtylmtorg);
	}
	break;
/*
...
#define HAL_NAV_UPPER_UNIT	128	/* micro-second */
		...
		rtw_write8(adapter, REG_NAV_CTRL_8733B + 2, (u8)usNavUpper);
	}
	break;
...
*/
	...
	case HW_VAR_SOUNDING_ENTER:
		rtl8733b_phy_bf_enter(adapter, (struct sta_info*)val);
		break;

	case HW_VAR_SOUNDING_LEAVE:
		rtl8733b_phy_bf_leave(adapter, val);
		break;
/*
...
*/
	case HW_VAR_SOUNDING_STATUS:
		rtl8733b_phy_bf_sounding_status(adapter, *val);
		break;
/*
...
*/
	case HW_VAR_SOUNDING_SET_GID_TABLE:
		rtl8733b_phy_bf_set_gid_table(adapter, (struct beamformer_entry*)val);
		break;
...
struct bcn_qinfo_8733b {
	u16 head:8;
	u16 pkt_num:8;
};

void dump_qinfo(void *sel, struct qinfo_8733b *info, const char *tag)
{
...
void dump_bcn_qinfo(void *sel, struct bcn_qinfo_8733b *info, const char *tag)
{
...
static void dump_mac_qinfo(void *sel, _adapter *adapter)
{
	...
	q0_info = rtw_read32(adapter, REG_Q0_INFO_8733B);
	q1_info = rtw_read32(adapter, REG_Q1_INFO_8733B);
	q2_info = rtw_read32(adapter, REG_Q2_INFO_8733B);
	q3_info = rtw_read32(adapter, REG_Q3_INFO_8733B);
	q4_info = rtw_read32(adapter, REG_Q4_INFO_8733B);
	q5_info = rtw_read32(adapter, REG_Q5_INFO_8733B);
	q6_info = rtw_read32(adapter, REG_Q6_INFO_8733B);
	q7_info = rtw_read32(adapter, REG_Q7_INFO_8733B);
	mg_q_info = rtw_read32(adapter, REG_MGQ_INFO_8733B);
	hi_q_info = rtw_read32(adapter, REG_HIQ_INFO_8733B);
	bcn_q_info = rtw_read16(adapter, REG_BCNQ_INFO_8733B);

	dump_qinfo(sel, (struct qinfo_8733b *)&q0_info, "Q0 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q1_info, "Q1 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q2_info, "Q2 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q3_info, "Q3 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q4_info, "Q4 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q5_info, "Q5 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q6_info, "Q6 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&q7_info, "Q7 ");
	dump_qinfo(sel, (struct qinfo_8733b *)&mg_q_info, "MG ");
	dump_qinfo(sel, (struct qinfo_8733b *)&hi_q_info, "HI ");
	dump_bcn_qinfo(sel, (struct bcn_qinfo_8733b *)&bcn_q_info, "BCN ");
}
...
static void dump_mac_txfifo(void *sel, _adapter *adapter)
{
	...
	rqpn = rtw_read32(adapter, REG_FIFOPAGE_INFO_8733B);
	rqpn_npq = rtw_read32(adapter, REG_RQPN_NPQ_8733B);
...
void rtl8733b_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	rtl8733b_gethwreg(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	rtl8733b_gethwreg(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	rtl8733b_gethwreg(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	rtl8733b_gethwreg(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void rtl8733b_gethwreg(PADAPTER adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE hal;
	u8 val8;
	u16 val16;
...
*/
	case HW_VAR_TXPAUSE:
		*val = rtw_read8(adapter, REG_TXPAUSE_8733B);
		break;
/*
	case HW_VAR_BCN_FUNC:
	case HW_VAR_MLME_DISCONNECT:
		...
		if (rtw_is_surprise_removed(adapter) ||
			...
			*val = _TRUE;
			...
			if (adapter_to_pwrctl(adapter)->wowlan_mode == _TRUE)
				val32 &= (BIT_UCMDINT_EN_8733B | BIT_BCMDINT_EN_8733B);
			else
				val32 &= (BIT_UCMDINT_EN_8733B | BIT_BCMDINT_EN_8733B | BIT_TIMPSR_EN_8733B);
...
*/
	case HW_VAR_SYS_CLKR:
		*val = rtw_read8(adapter, REG_SYS_CLK_CTRL_8733B);
		break;
/*
	case HW_VAR_NAV_UPPER:
	case HW_VAR_RPT_TIMER_SETTING:
...
*/
	case HW_VAR_CHK_HI_QUEUE_EMPTY:
		val16 = rtw_read16(adapter, REG_TXPKT_EMPTY_8733B);
		*val = (val16 & BIT_HQQ_EMPTY_8733B) ? _TRUE : _FALSE;
		break;
	case HW_VAR_CHK_MGQ_CPU_EMPTY:
		val16 = rtw_read16(adapter, REG_TXPKT_EMPTY_8733B);
		*val = (val16 & BIT_MGQ_CPU_EMPTY_8733B) ? _TRUE : _FALSE;
		break;
/*
 ...
 */
u8 rtl8733b_sethaldefvar(PADAPTER adapter, HAL_DEF_VARIABLE variable, void *pval)
{
	PHAL_DATA_TYPE hal;
	u8 bResult;
...
void rtl8733b_ra_info_dump(_adapter *padapter, void *sel)
{
	u8 mac_id;
 ...
 */
u8 rtl8733b_gethaldefvar(PADAPTER adapter, HAL_DEF_VARIABLE variable, void *pval)
{
	PHAL_DATA_TYPE hal;
	struct dvobj_priv *d;
	u8 bResult;
...
*/
	...
	case HAL_DEF_RX_PACKET_OFFSET:
		val32 = rtl8733b_get_rx_desc_size(adapter);
		val8 = rtl8733b_get_rx_drv_info_size(adapter);
		*((u32 *)pval) = val32 + val8;
		break;
/*
	case HAL_DEF_RX_DMA_SZ_WOW:
...
*/
	...
	case HW_VAR_MAX_RX_AMPDU_FACTOR:
		/* 8733B RX FIFO is 24KB */
		*(HT_CAP_AMPDU_FACTOR *)pval = MAX_AMPDU_FACTOR_16K;
		break;

	case HW_DEF_RA_INFO_DUMP:
		rtl8733b_ra_info_dump(adapter, pval);
		break;
/*
	case HAL_DEF_DBG_DUMP_TXPKT:
	case HAL_DEF_TX_PAGE_SIZE:
...
*/
	...
	case HAL_DEF_TX_BUFFER_LAST_ENTRY:
		*(u8 *)pval = LAST_ENTRY_OF_TX_PKT_BUFFER_8733B;
		break;
...
void rtl8733b_fill_txdesc_sectype(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && !pattrib->bswenc) {
		...
		case _TKIP_:
		case _TKIP_WTMIC_:
			SET_TX_DESC_SEC_TYPE_8733B(ptxdesc, 0x1);
			break;
#ifdef CONFIG_WAPI_SUPPORT
		case _SMS4_:
			SET_TX_DESC_SEC_TYPE_8733B(ptxdesc, 0x2);
			break;
#endif
		case _AES_:
			SET_TX_DESC_SEC_TYPE_8733B(ptxdesc, 0x3);
			break;
		case _CCMP_256_:
		case _GCMP_:
		case _GCMP_256_:
		...
		case _NO_PRIVACY_:
		default:
			SET_TX_DESC_SEC_TYPE_8733B(ptxdesc, 0x0);
			break;
		}
	}
}

void rtl8733b_fill_txdesc_vcs(PADAPTER adapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	if (pattrib->vcs_mode) {
		...
		case RTS_CTS:
			SET_TX_DESC_RTSEN_8733B(ptxdesc, 1);
			break;
		case CTS_TO_SELF:
			SET_TX_DESC_CTS2SELF_8733B(ptxdesc, 1);
			break;
		case NONE_VCS:
		default:
			break;
		...
		if (pmlmeinfo->preamble_mode == PREAMBLE_SHORT)
			SET_TX_DESC_RTS_SHORT_8733B(ptxdesc, 1);

		/* RTS Rate=24M */
		SET_TX_DESC_RTSRATE_8733B(ptxdesc, 0x8);

		/* compatibility for MCC consideration, use pmlmeext->cur_channel */
		if (pmlmeext->cur_channel > 14)
			/* RTS retry to rate OFDM 6M for 5G */
			SET_TX_DESC_RTS_RTY_LOWEST_RATE_8733B(ptxdesc, 4);
		else
			/* RTS retry to rate CCK 1M for 2.4G */
			SET_TX_DESC_RTS_RTY_LOWEST_RATE_8733B(ptxdesc, 0);
	}
}

u8 rtl8733b_bw_mapping(PADAPTER adapter, struct pkt_attrib *pattrib)
{
...
u8 rtl8733b_sc_mapping(PADAPTER adapter, struct pkt_attrib *pattrib)
{
...
void rtl8733b_fill_txdesc_phy(PADAPTER adapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if (pattrib->ht_en) {
		/* Set Bandwidth and sub-channel settings. */
		SET_TX_DESC_DATA_BW_8733B(ptxdesc, rtl8733b_bw_mapping(adapter, pattrib));
		SET_TX_DESC_DATA_SC_8733B(ptxdesc, rtl8733b_sc_mapping(adapter, pattrib));
	}
}

/**
 * rtl8733b_fill_txdesc_tx_rate() - Set rate in tx description
 ...
 */
static void rtl8733b_fill_txdesc_tx_rate(struct _ADAPTER *adapter,
				...
				u8 rate, u8 shrt, u8 fallback, u8 *desc)
{
	...
	SET_TX_DESC_USE_RATE_8733B(desc, 1);
	SET_TX_DESC_DATARATE_8733B(desc, rate);
	SET_TX_DESC_DATA_SHORT_8733B(desc, shrt);
	SET_TX_DESC_DISDATAFB_8733B(desc, disfb);

	/* HT MCS rate can't support bandwidth higher than 40MHz */
	bw = GET_TX_DESC_DATA_BW_8733B(desc);
	if (((rate >= DESC_RATEMCS0) && (rate <= DESC_RATEMCS31)) && (bw > 1)) {
		...
		rtl8733b_fill_txdesc_phy(adapter, attrib, desc);
	}
}

void rtl8733b_fill_txdesc_force_bmc_camid(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && (!pattrib->bswenc)
	    && (pattrib->bmc_camid != INVALID_SEC_MAC_CAM_ID)) {
		SET_TX_DESC_EN_DESC_ID_8733B(ptxdesc, 1);
		SET_TX_DESC_MACID_8733B(ptxdesc, pattrib->bmc_camid);
	}
}
 ...
 */
void rtl8733b_fill_txdesc_bf(struct xmit_frame *frame, u8 *desc)
{
	...
	SET_TX_DESC_G_ID_8733B(desc, attrib->txbf_g_id);
	SET_TX_DESC_P_AID_8733B(desc, attrib->txbf_p_aid);

	SET_TX_DESC_MU_DATARATE_8733B(desc, MRateToHwRate(attrib->rate));
	/*SET_TX_DESC_MU_RC_8733B(desc, 0);*/

	/* Force to disable STBC when txbf is enabled */
	if (attrib->txbf_p_aid && attrib->stbc)
		SET_TX_DESC_DATA_STBC_8733B(desc, 0);
#endif /* CONFIG_BEAMFORMING */
}
 ...
 */
void rtl8733b_fill_txdesc_mgnt_bf(struct xmit_frame *frame, u8 *desc)
{
#ifndef CONFIG_BEAMFORMING
	return;
#else /* CONFIG_BEAMFORMING */
	...
	/*SET_TX_DESC_TXPKTSIZE_8733B(desc, pattrib->last_txcmdsz);*/
	/*SET_TX_DESC_OFFSET_8733B(desc, HALMAC_TX_DESC_SIZE_8733B);*/
	SET_TX_DESC_DISRTSFB_8733B(desc, 1);
	SET_TX_DESC_DISDATAFB(desc, 1);
	/*SET_TX_DESC_SW_SEQ_8733B(desc, pattrib->seqnum);*/
	SET_TX_DESC_DATA_BW_8733B(desc, rtl8733b_bw_mapping(adapter, attrib));
	SET_TX_DESC_SIGNALING_TA_PKT_SC_8733B(desc,
					rtl8733b_sc_mapping(adapter, attrib));
	/*SET_TX_DESC_RTY_LMT_EN_8733B(ptxdesc, 1);*/
	SET_TX_DESC_RTS_DATA_RTY_LMT_8733B(desc, 5);
	SET_TX_DESC_NDPA_8733B(desc, 1);
	SET_TX_DESC_NAVUSEHDR_8733B(desc, 1);
	/*SET_TX_DESC_QSEL_8733B(desc, QSLT_MGNT);*/
	/*
	 * NSS2MCS0 for VHT
	 * MCS8 for HT
	 */
	SET_TX_DESC_DATARATE_8733B(desc, MRateToHwRate(attrib->rate));
	/*SET_TX_DESC_USE_RATE_8733B(desc, 1);*/
	/*SET_TX_DESC_MACID_8733B(desc, pattrib->mac_id);*/ /* ad-hoc mode */
	/*SET_TX_DESC_G_ID_8733B(desc, 63);*/
	/*
	 ...
	 */
	/*SET_TX_DESC_P_AID_8733B(desc, pattrib->txbf_p_aid);*/
	SET_TX_DESC_SND_PKT_SEL_8733B(desc, attrib->bf_pkt_type);
#endif /* CONFIG_BEAMFORMING */
}

void rtl8733b_cal_txdesc_chksum(PADAPTER adapter, u8 *ptxdesc)
{
...
void rtl8733b_prepare_mp_txdesc(PADAPTER adapter, struct mp_priv *pmp_priv)
{
	u8 *desc;
	struct pkt_attrib *attrib;
	u32 pkt_size;
	...
	SET_TX_DESC_TXPKTSIZE_8733B(desc, pkt_size);

	offset = desc_size;
	SET_TX_DESC_OFFSET_8733B(desc, offset);
#if defined(CONFIG_PCI_HCI)
	SET_TX_DESC_PKT_OFFSET_8733B(desc, 0); /* 8733BE pkt_offset is 0 */
#else
	SET_TX_DESC_PKT_OFFSET_8733B(desc, 1);
#endif

	if (bmcast)
		SET_TX_DESC_BMC_8733B(desc, 1);

	SET_TX_DESC_MACID_8733B(desc, attrib->mac_id);
	SET_TX_DESC_RATE_ID_8733B(desc, attrib->raid);
	SET_TX_DESC_QSEL_8733B(desc, attrib->qsel);

	if (pmp_priv->preamble)
		SET_TX_DESC_DATA_SHORT_8733B(desc, 1);

	if (!attrib->qos_en)
		SET_TX_DESC_EN_HWSEQ_8733B(desc, 1);
	else
		SET_TX_DESC_SW_SEQ_8733B(desc, attrib->seqnum);

	if (pmp_priv->bandwidth <= CHANNEL_WIDTH_160)
		SET_TX_DESC_DATA_BW_8733B(desc, pmp_priv->bandwidth);
	else {
		RTW_ERR("%s: unknown bandwidth %d, use 20M\n",
			 __FUNCTION__, pmp_priv->bandwidth);
		SET_TX_DESC_DATA_BW_8733B(desc, CHANNEL_WIDTH_20);
	}

	SET_TX_DESC_DISDATAFB_8733B(desc, 1);
	SET_TX_DESC_USE_RATE_8733B(desc, 1);
	SET_TX_DESC_DATARATE_8733B(desc, pmp_priv->rateidx);
}
...
void fill_default_txdesc(struct xmit_frame *pxmitframe, u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		rtl8733b_fill_txdesc_sectype(pattrib, pbuf);
		rtl8733b_fill_txdesc_vcs(adapter, pattrib, pbuf);

		if (bmcst)
			rtl8733b_fill_txdesc_force_bmc_camid(pattrib, pbuf);
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				SET_TX_DESC_AGG_EN_8733B(pbuf, 1);
				SET_TX_DESC_MAX_AGG_NUM_8733B(pbuf, 0x1F);
				SET_TX_DESC_AMPDU_DENSITY_8733B(pbuf, pattrib->ampdu_spacing);
			} else
				SET_TX_DESC_BK_8733B(pbuf, 1);

			rtl8733b_fill_txdesc_phy(adapter, pattrib, pbuf);

			/* compatibility for MCC consideration, use pmlmeext->cur_channel */
			if (!bmcst) {
				if (pmlmeext->cur_channel > 14)
					/* for 5G, OFDM 6M */
					SET_TX_DESC_DATA_RTY_LOWEST_RATE_8733B(pbuf, 4);
				else
					/* for 2.4G, CCK 1M */
					SET_TX_DESC_DATA_RTY_LOWEST_RATE_8733B(pbuf, 0);
			}

			if (hal->fw_ractrl == _FALSE)
				rtl8733b_fill_txdesc_tx_rate(adapter, pattrib,
					hal->INIDATA_RATE[pattrib->mac_id] & 0x7F,
					hal->INIDATA_RATE[pattrib->mac_id] & BIT(7) ? 1 : 0,
					1, pbuf);

			if (bmcst) {
				SET_TX_DESC_SW_DEFINE_8733B(pbuf, 0x01);
				rtl8733b_fill_txdesc_bmc_tx_rate(pattrib, pbuf);
			}
			...
			if (pattrib->stbc)
				SET_TX_DESC_DATA_STBC_8733B(pbuf, 1);

#ifdef CONFIG_CMCC_TEST
			SET_TX_DESC_DATA_SHORT_8733B(pbuf, 1); /* use cck short premble */
#endif

#ifdef CONFIG_WMMPS_STA
			if (pattrib->trigger_frame)
				SET_TX_DESC_TRI_FRAME_8733B (pbuf, 1);
			...
			if((pattrib->ether_type == ETH_P_ARP) &&
				...
				SET_TX_DESC_DATARATE_8733B(pbuf, MRateToHwRate(IEEE80211_OFDM_RATE_6MB));
			 else
#endif/*CONFIG_IP_R_MONITOR*/
				SET_TX_DESC_DATARATE_8733B(pbuf, MRateToHwRate(pmlmeext->tx_rate));
		...
		SET_TX_DESC_DMA_TXAGG_NUM_8733B(pbuf, pxmitframe->agg_num);
#endif
...
#endif
			SET_TX_DESC_SPE_RPT_8733B(pbuf, 1);
			SET_TX_DESC_SW_DEFINE_8733B(pbuf, (u8)(GET_PRIMARY_ADAPTER(adapter)->xmitpriv.seq_no));
		}
		...
		SET_TX_DESC_MBSSID_8733B(pbuf, pattrib->mbssid & 0xF);
		...
		if (pattrib->retry_ctrl == _TRUE)
			...
			SET_TX_DESC_RTS_DATA_RTY_LMT_8733B(pbuf, 12);

		rtl8733b_fill_txdesc_mgnt_bf(pxmitframe, pbuf);
	...
	SET_TX_DESC_TXPKTSIZE_8733B(pbuf, pattrib->last_txcmdsz);

	{
		...
		SET_TX_DESC_PKT_OFFSET_8733B(pbuf, pkt_offset);
		SET_TX_DESC_OFFSET_8733B(pbuf, offset);
#ifdef CONFIG_TCP_CSUM_OFFLOAD_TX
	if (pattrib->hw_csum == 1) {
		int offset = 48 + pkt_offset*8 + 24;

		SET_TX_DESC_OFFSET_8733B(pbuf, offset);
		SET_TX_DESC_CHK_EN_8733B(pbuf, 1);
		SET_TX_DESC_WHEADER_LEN_8733B(pbuf, (pattrib->hdrlen + pattrib->iv_len + XATTRIB_GET_MCTRL_LEN(pattrib))>>1);
	}
#endif
	}

	if (bmcst)
		SET_TX_DESC_BMC_8733B(pbuf, 1);
	...
	SET_TX_DESC_MULTI_PORT_ID_8733B(pbuf, hw_port);
	...
	rtl8733b_fill_txdesc_bf(pxmitframe, pbuf);
}
 ...
 */
void rtl8733b_update_txdesc(struct xmit_frame *pxmitframe, u8 *pbuf)
{
	fill_default_txdesc(pxmitframe, pbuf);
	rtl8733b_cal_txdesc_chksum(pxmitframe->padapter, pbuf);
}
			     ...
			     u8 IsPsPoll, u8 IsBTQosNull, u8 bDataFrame)
{
	...
	SET_TX_DESC_OFFSET_8733B(pDesc, desc_size);

	SET_TX_DESC_TXPKTSIZE_8733B(pDesc, BufferLen);
	SET_TX_DESC_QSEL_8733B(pDesc, QSLT_MGNT); /* Fixed queue of Mgnt queue */

	if (pmlmeext->cur_wireless_mode & WIRELESS_11B)
		SET_TX_DESC_RATE_ID_8733B(pDesc, RATEID_IDX_B);
	else
		SET_TX_DESC_RATE_ID_8733B(pDesc, RATEID_IDX_G);

	/* Set NAVUSEHDR to prevent Ps-poll AId filed to be changed to error vlaue by HW */
	if (_TRUE == IsPsPoll)
		SET_TX_DESC_NAVUSEHDR_8733B(pDesc, 1);
	else {
		SET_TX_DESC_EN_HWSEQ_8733B(pDesc, 1);
		SET_TX_DESC_HW_SSN_SEL_8733B(pDesc, pxmitpriv->hw_ssn_seq_no);/*pattrib->hw_ssn_sel*/
	}

	if (_TRUE == IsBTQosNull)
		SET_TX_DESC_BT_NULL_8733B(pDesc, 1);

	SET_TX_DESC_USE_RATE_8733B(pDesc, 1);
	SET_TX_DESC_DATARATE_8733B(pDesc, MRateToHwRate(pmlmeext->tx_rate));
	...
	if (IsPsPoll == _FALSE && IsBTQosNull == _FALSE && bDataFrame == _FALSE) {
		if (rtw_hal_check_mcc_status(adapter, MCC_STATUS_PROCESS_MCC_START_SETTING)) {
			...
			if (rty_num != 0) {
				SET_TX_DESC_RTY_LMT_EN_8733B(pDesc, 1);
				SET_TX_DESC_RTS_DATA_RTY_LMT_8733B(pDesc, rty_num);
			}
		}
	}
#endif
	...
	if (_TRUE == bDataFrame) {
		...
		switch (EncAlg) {
		case _NO_PRIVACY_:
			SET_TX_DESC_SEC_TYPE_8733B(pDesc, 0x0);
			break;
		case _WEP40_:
		case _WEP104_:
		case _TKIP_:
			SET_TX_DESC_SEC_TYPE_8733B(pDesc, 0x1);
			break;
		case _SMS4_:
			SET_TX_DESC_SEC_TYPE_8733B(pDesc, 0x2);
			break;
		case _AES_:
			SET_TX_DESC_SEC_TYPE_8733B(pDesc, 0x3);
			break;
		case _CCMP_256_:
		case _GCMP_:
		case _GCMP_256_:
	...
	SET_TX_DESC_MULTI_PORT_ID_8733B(pDesc, hw_port);

	/*
	 * USB interface drop packet if the checksum of descriptor isn't correct.
	 * Using this checksum can let hardware recovery from packet bulk out error (e.g. Cancel URC, Bulk out error.).
	 */
	rtl8733b_cal_txdesc_chksum(adapter, pDesc);
}

void rtl8733b_dbg_dump_tx_desc(PADAPTER adapter, int frame_tag, u8 *ptxdesc)
{
	u8 bDumpTxPkt;
#if defined(CONFIG_MAC_LOOPBACK_DRIVER) || defined(CONFIG_TWO_MAC_DRIVER)
	u8 bDumpTxDesc = _TRUE;
#else
...
#endif
	/* 8733B TX SIZE = 48(HALMAC_TX_DESC_SIZE_8733B) */
	if (_TRUE == bDumpTxDesc) {
		RTW_INFO("=====================================\n");
		RTW_INFO("TXDESC_SIZE = %u\n", TXDESC_SIZE);
...
void rtl8733b_rxdesc2attribute(struct rx_pkt_attrib *a, u8 *desc)
{
	...
	/* Get from RX DESC */
	a->pkt_len = (u16)GET_RX_DESC_PKT_LEN_8733B(desc);
	a->pkt_rpt_type = GET_RX_DESC_C2H_8733B(desc) ? C2H_PACKET : NORMAL_RX;

	if (a->pkt_rpt_type == NORMAL_RX) {
		a->crc_err = (u8)GET_RX_DESC_CRC32_8733B(desc);
		a->icv_err = (u8)GET_RX_DESC_ICV_ERR_8733B(desc);
		a->drvinfo_sz = (u8)GET_RX_DESC_DRV_INFO_SIZE_8733B(desc) << 3;
		a->encrypt = (u8)GET_RX_DESC_SECURITY_8733B(desc);
		a->qos = (u8)GET_RX_DESC_QOS_8733B(desc);
		a->shift_sz = (u8)GET_RX_DESC_SHIFT_8733B(desc);
		a->physt = (u8)GET_RX_DESC_PHYST_8733B(desc);
		a->bdecrypted = (u8)GET_RX_DESC_SWDEC_8733B(desc) ? 0 : 1;

		a->priority = (u8)GET_RX_DESC_TID_8733B(desc);
		a->amsdu = (u8)GET_RX_DESC_AMSDU_8733B(desc);
		a->mdata = (u8)GET_RX_DESC_MD_8733B(desc);
		a->mfrag = (u8)GET_RX_DESC_MF_8733B(desc);

		a->seq_num = (u16)GET_RX_DESC_SEQ_8733B(desc);
		a->frag_num = (u8)GET_RX_DESC_FRAG_8733B(desc);

		a->data_rate = (u8)GET_RX_DESC_RX_RATE_8733B(desc);
		a->ampdu = (u8)GET_RX_DESC_PAGGR_8733B(desc);
		a->ampdu_eof = (u8)GET_RX_DESC_RX_EOF_8733B(desc);
		a->ppdu_cnt = (u8)GET_RX_DESC_PPDU_CNT_8733B(desc);
		a->free_cnt = (u32)GET_RX_DESC_TSFL_8733B(desc);

#ifdef CONFIG_TCP_CSUM_OFFLOAD_RX
		/* RX TCP checksum offload related variables */
		a->csum_valid = (u8)GET_RX_DESC_CHK_VLD_8733B(desc);
		a->csum_err = (u8)GET_RX_DESC_CHKERR_8733B(desc);
#endif /* CONFIG_TCP_CSUM_OFFLOAD_RX */
	}
}

void rtl8733b_query_rx_desc(union recv_frame *precvframe, u8 *pdesc)
{
	rtl8733b_rxdesc2attribute(&precvframe->u.hdr.attrib, pdesc);
}

void rtl8733b_set_hal_ops(PADAPTER adapter)
{
	struct hal_com_data *hal;
	struct hal_ops *ops;
...
*/
	ops->read_adapter_info = rtl8733b_read_efuse;
	ops->hal_power_on = rtl8733b_power_on;
	ops->hal_power_off = rtl8733b_power_off;
	ops->hal_init = rtl8733b_init;
	ops->hal_deinit = rtl8733b_deinit;
	ops->dm_init = rtl8733b_phy_init_dm_priv;
	ops->dm_deinit = rtl8733b_phy_deinit_dm_priv;
...
#endif
*/
	ops->run_thread = rtl8733b_run_thread;
	ops->cancel_thread = rtl8733b_cancel_thread;
...
*/
	...
	/*** DM section ***/
	ops->set_chnl_bw_handler = rtl8733b_set_channel_bw;

	ops->set_tx_power_level_handler = rtl8733b_set_tx_power_level;
	ops->set_txpwr_done = rtl8733b_set_txpwr_done;
	ops->set_tx_power_index_handler = rtl8733b_set_tx_power_index;
	ops->get_tx_power_index_handler = hal_com_get_txpwr_idx;

	ops->hal_dm_watchdog = rtl8733b_phy_haldm_watchdog;

	ops->set_hw_reg_handler = rtl8733b_sethwreg;
	ops->GetHwRegHandler = rtl8733b_gethwreg;
	ops->get_hal_def_var_handler = rtl8733b_gethaldefvar;
	ops->SetHalDefVarHandler = rtl8733b_sethaldefvar;
...
*/
	ops->read_bbreg = rtl8733b_read_bb_reg;
	ops->write_bbreg = rtl8733b_write_bb_reg;
	ops->read_rfreg = rtl8733b_read_rf_reg;
	ops->write_rfreg = rtl8733b_write_rf_reg;
	ops->read_wmmedca_reg = rtl8733b_read_wmmedca_reg;
...
*/
	...
	ops->hal_notch_filter = rtl8733b_notch_filter_switch;
	ops->hal_mac_c2h_handler = rtl8733b_c2h_handler;
	ops->fill_h2c_cmd = rtl8733b_fillh2ccmd;
	ops->fill_fake_txdesc = fill_fake_txdesc;
	ops->fw_dl = rtl8733b_fw_dl;
...
*/
	...
	ops->init_mac_register = rtl8733b_phy_init_mac_register;
	ops->init_phy = rtl8733b_phy_init;
	ops->reqtxrpt = rtl8733b_req_txrpt_cmd;
}
